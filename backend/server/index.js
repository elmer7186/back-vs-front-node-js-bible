...
const { Tecnology } = require('../models');
...
server.get("/api/technologies", async (req, res) => {
    let technologies = await Tecnology.find();
...
server.get("/api/technologies/:id", async (req, res) => {
    const { id } = req.params;
    let technology = await Tecnology.findById(id);
    technology.logo = `${req.protocol}://${req.headers.host}/img/${technology.logo}`;
...
server.get("/api/technologies/search/:name", async (req, res) => {
    ...
    let technologies = await Tecnology.find({ name: { $regex: new RegExp(name, 'i') } });
    technologies = technologies.map((technology) => {
        technology.logo = `${req.protocol}://${req.headers.host}/img/${technology.logo}`;
        return technology;
    });
